...
export default function SelectWallet({ variant = "ctaBig" }: { variant?: "nav" | "ctaBig" }) {
  ...
  useEffect(() => {
    ...
    const savedWalletName = localStorage.getItem("shade_wallet_name");
    if (savedWalletName && !isConnected) {
      setTimeout(async () => {
        try {
          const allWallets = store.getWallets() as any as WalletWithStarknetFeatures[];
          const saved = allWallets.find((w) => normalizeId(w.name) === normalizeId(savedWalletName));
          if (saved) {
            console.log("Auto-reconnecting to", saved.name);
            await handleSelectedWallet(saved);
          }
        } catch (err: any) {
  ...
  async function handleSelectedWallet(selectedWallet: WalletWithStarknetFeatures) {
    ...
    // Save wallet name for auto-reconnect on next page load.
    try { localStorage.setItem("shade_wallet_name", selectedWallet.name); } catch {}

    // Step 1: Request accounts first — this wakes up the wallet extension
    // and prompts the user to approve. Without this, WalletAccountV6.connect()
    // can hang on stale sessions after server restarts.
    // Add a 15s timeout — Ready wallet can hang on stale sessions.
    let result: any;
    try {
      result = await Promise.race([
        walletV6.requestAccounts(selectedWallet),
        new Promise<string>((_, reject) =>
          setTimeout(() => reject(new Error(`${selectedWallet.name} connection timed out. Try locking and unlocking the wallet, then refresh the page.`)), 15_000)
        ),
      ]);
    } catch (err: any) {
      console.error("requestAccounts failed:", err?.message);
      throw err;
    }
    ...
    // Step 2: Now connect the WalletAccount (session is already active).
    const provider = myFrontendProviders[myFrontendProviderIndex] ?? myFrontendProviders[2];
    try {
      const myWA = await Promise.race([
        WalletAccountV6.connect(provider, selectedWallet),
        new Promise<any>((_, reject) =>
          setTimeout(() => reject(new Error("WalletAccount connect timed out")), 10_000)
        ),
      ]);
      setMyWalletAccount(myWA);
      console.log("WalletAccount created=", myWA);
    } catch (err: any) {
      console.log("WalletAccount.connect failed:", err?.message);
      // Non-fatal — we can still operate via direct wallet API calls.
    }
    ...
    if (isConnected && address) {
      return (
        <button
          ...
          onClick={() => {
            setConnected(false);
            try { localStorage.removeItem("shade_wallet_name"); } catch {}
          }}
          title="Disconnect"
        >
